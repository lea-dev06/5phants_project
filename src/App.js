...
class App extends React.Component {
  ...
  setPriceCurrency = (id_price_currency) => {
    console.log('SETPRICECURRRRRRRR', id_price_currency)
  ...
  publishData = async () => {
    if (this.state.DATAS.product && this.state.DATAS.title) {
    const DATAS = {
      ...this.state.DATAS, posted_at_date: new Date()
    }
    await this.setState ({ DATAS, showData: true })
    // alert('Your data was published!')
    console.log(this.state.DATAS)
  } else {
    alert('Please fill in the product and the title field!')
  }
    ...
    if (this.state.offersRender) {
      ...
      return (
        <Container fluid={true}>
          <Row>
            {this.state.showSendData && <PostForm clearData={this.clearData} publishData={this.publishData}/>}
          </Row>
          <Row>
            <Col id="main" xl={12}>
